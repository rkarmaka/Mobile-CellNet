...
from modules.utils import mobile_bottleneck

########################################################################################################################
# U-Net
def Unet(image_size):
    f = [64, 128, 256, 512, 1024]
    inp = Input((image_size))  # 1
...
def Unet_new(image_size):
    ...
    # expansion
    e31 = func.expansion(bn, c3, num_filter=f[2]) # c1=480x320x1, p1=240x160x64
    e21 = func.expansion(e31, c2, num_filter=f[1])
    e11 = func.expansion(e21, c1, num_filter=f[0])

    out_temp = Conv2D(2, (3, 3), padding='SAME', activation='relu')(e11)
    out1 = Conv2D(1, (1, 1), padding='SAME', activation='sigmoid')(out_temp)

    # expansion
    e32 = func.expansion(bn, c3, num_filter=f[2])  # c1=480x320x1, p1=240x160x64
    e22 = func.expansion(e32, c2, num_filter=f[1])
    e12 = func.expansion(e22, c1, num_filter=f[0])

    out_temp = Conv2D(2, (3, 3), padding='SAME', activation='relu')(e12)
    out2 = Conv2D(1, (1, 1), padding='SAME', activation='sigmoid')(out_temp)
...
########################################################################################################################
# Mobile-CellNet
def mobile_cellNet(image_size, pre_trained=False):
    if pre_trained:
        ...
        c1 = mobile_bottleneck(x, 48, 8, strides=2)
        c1 = mobile_bottleneck(c1, 48, 8, strides=1)
        c1 = mobile_bottleneck(c1, 48, 8, strides=1)

        # second bottleneck layer
        c2 = mobile_bottleneck(c1, 96, 16, strides=2)
        c2 = mobile_bottleneck(c2, 96, 16, strides=1)
        c2 = mobile_bottleneck(c2, 96, 16, strides=1)

        c3 = mobile_bottleneck(c2, 144, 32, strides=2)
        c3 = mobile_bottleneck(c3, 144, 32, strides=1)
        c3 = mobile_bottleneck(c3, 144, 32, strides=1)

        c4 = mobile_bottleneck(c3, 144, 32, strides=2)
        c4 = mobile_bottleneck(c4, 144, 32, strides=1)
        c4 = mobile_bottleneck(c4, 144, 32, strides=1)

        c5 = mobile_bottleneck(c4, 144, 32, strides=2)
        c5 = mobile_bottleneck(c5, 144, 32, strides=1)
        c5 = mobile_bottleneck(c5, 144, 32, strides=1)
        c5 = mobile_bottleneck(c5, 144, 32, strides=1)
        c5 = mobile_bottleneck(c5, 144, 32, strides=1)
        c5 = mobile_bottleneck(c5, 144, 32, strides=1)
    ...
    e4 = mobile_bottleneck(e4, 144, 32, add=False)
    e4 = mobile_bottleneck(e4, 144, 32, strides=1)

    e3 = Conv2DTranspose(16, kernel_size=3, strides=2, padding='same')(e4)
    e3 = Concatenate()([e3, c3])
    e3 = mobile_bottleneck(e3, 96, 16, add=False)
    e3 = mobile_bottleneck(e3, 96, 16, strides=1)

    # upsampling 2
    e2 = Conv2DTranspose(16, kernel_size=3, strides=2, padding='same')(e3)
    e2 = Concatenate()([e2, c2])
    e2 = mobile_bottleneck(e2, 96, 16, add=False)
    e2 = mobile_bottleneck(e2, 96, 16, strides=1)
    ...
    e1 = mobile_bottleneck(e1, 48, 8, add=False)
    e1 = mobile_bottleneck(e1, 48, 8, strides=1)

    # upsampling 1
    e0 = Conv2DTranspose(8, kernel_size=3, strides=2, padding='same')(e1)
    e0 = Concatenate()([e0, x])
    e0 = mobile_bottleneck(e0, 48, 8, add=False)
    e0 = mobile_bottleneck(e0, 48, 8, strides=1)
